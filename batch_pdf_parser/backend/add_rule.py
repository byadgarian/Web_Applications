...
def add_rule(field_data):
    try:
        # Generate INSERT statement and handle errors
        command = "INSERT INTO pdf_parser.rules ("
        for key in field_data.keys():
            if key == 'destination': continue
            elif key == 'new_rule_name': command += "rule_name" + ", "
            else: command += str(key) + ", "
        command = command[:-2]
        command += ") VALUES ('"
        for key in field_data.keys(): 
            if key == 'destination': continue
...
def main():
    ...
    error_message, success_message = add_rule(field_data)
    print()
